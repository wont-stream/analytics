...
describe("Operating systems", () => {
  for (const operatingSystemTest of operatingSystemTests) {
    test(`${operatingSystemTest.os.name} ${
      operatingSystemTest.os.version || ""
    }`, () => {
      const result = deviceDetector.parse(operatingSystemTest.user_agent)
        .os as OperatingSystemResult;
      ...
      if (!operatingSystemTest.os.version) {
        ...
        expect(result.version).toEqual(
          formatVersion(operatingSystemTest.os.version, 1)
        );
      }
